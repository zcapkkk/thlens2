...
coeffs = [2.459406267497484   2.430296972020067   2.417060174325630   2.413452222152528   2.454022316570370];
...
for angles = 1:length(xangles)
    ...
    for i = 1:steps
        T = steps - i;
        onethickness =l.phaseprofile(coeffs, antenna_r);
        onelens = l.makelensfromthickness(onethickness, antenna_r, 1);
        apu1 = l.lenspropagate(u0, onelens, 0, z2);
        i_error = loptim.field2error(apu1, ndemou1, dblim);
        % cascade
        for j = 1:length(coeffs)
            new_coeffs = coeffs;
            new_coeffs(j) = new_coeffs(j) + lr*rand(1,1);
            onelens = l.makephaselens(new_coeffs, antenna_r, 1);
            apu1 = l.lenspropagate(u0, onelens, 0, z2);
...
subplot(131);
pf = k0*(sqrt(p.x.^2 + z1^2) + sqrt(p.x.^2 + z2^2));
...
title(["My Lens at ", int2str(hornangle)," degrees"]);
axis square;
colormap jet;
caxis([-20 0]);
colorbar;
